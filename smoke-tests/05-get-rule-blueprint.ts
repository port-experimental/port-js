/**
 * Smoke Test: Fetch the '_rule' blueprint from Port API
 * Uses credentials from .env.local
 * 
 * Run: pnpm tsx smoke-tests/05-get-rule-blueprint.ts
 */

import { PortClient } from '../src/client';

async function testRuleBlueprint() {
  try {
    ...
    // Fetch the '_rule' blueprint (built-in system blueprint)
    console.log('📏 Fetching "_rule" blueprint...');
    const blueprint = await client.blueprints.get('_rule');
    ...
    console.log('');
    console.log('✅ Test successful! Rule blueprint retrieved correctly.');
    console.log('');
    console.log('ℹ️  Note: _rule is a built-in Port system blueprint for defining scorecard rules.');
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('credentials')) {
        ...
        console.error('');
        console.error('💡 Hint: The "_rule" blueprint does not exist.');
        console.error('   This is unusual as _rule is a built-in Port system blueprint.');
