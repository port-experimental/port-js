/**
 * Smoke Test: Fetch the '_user' blueprint from Port API
 * Uses credentials from .env.local
 * 
 * Run: pnpm tsx smoke-tests/03-get-user-blueprint.ts
 */

import { PortClient } from '../src/client';

async function testUserBlueprint() {
  try {
    ...
    // Fetch the '_user' blueprint (built-in blueprint)
    console.log('👤 Fetching "_user" blueprint...');
    const blueprint = await client.blueprints.get('_user');
    ...
    console.log('');
    console.log('✅ Test successful! User blueprint retrieved correctly.');
    console.log('');
    console.log('ℹ️  Note: _user is a built-in Port blueprint representing users/people.');
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('credentials')) {
        ...
        console.error('');
        console.error('💡 Hint: The "_user" blueprint does not exist.');
        console.error('   This is unusual as _user is a built-in Port blueprint.');
