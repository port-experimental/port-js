/**
 * Smoke Test: Fetch the '_scorecard' blueprint from Port API
 * Uses credentials from .env.local
 * 
 * Run: pnpm tsx smoke-tests/04-get-scorecard-blueprint.ts
 */

import { PortClient } from '../src/client';

async function testScorecardBlueprint() {
  try {
    ...
    // Fetch the '_scorecard' blueprint (built-in system blueprint)
    console.log('📊 Fetching "_scorecard" blueprint...');
    const blueprint = await client.blueprints.get('_scorecard');
    ...
    console.log('');
    console.log('✅ Test successful! Scorecard blueprint retrieved correctly.');
    console.log('');
    console.log('ℹ️  Note: _scorecard is a built-in Port system blueprint for evaluating entities.');
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('credentials')) {
        ...
        console.error('');
        console.error('💡 Hint: The "_scorecard" blueprint does not exist.');
        console.error('   This is unusual as _scorecard is a built-in Port system blueprint.');
